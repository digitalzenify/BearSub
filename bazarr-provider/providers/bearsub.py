...
class BearSubSubtitle(Subtitle):
    provider_name = "bearsub"
    ...
    def get_matches(self, video):
        ...
        if self.release_info:
            try:
                ...
                logger.debug("bearsub: update_matches failed for %r: %s", self.release_info, e)

        self.matches = matches
        return matches


class BearSubProvider(Provider):
    provider_name = "bearsub"
    ...
    def __init__(self, api_key=None, api_url=None, **kwargs):
        self.api_key = (api_key or os.getenv("BEARSUB_API_KEY", "")).strip()
        self.api_v1 = _normalize_api_base(
            api_url or os.getenv("BEARSUB_API_URL", "") or "http://localhost:8765"
        )
    ...
    def _search(self, imdb_id, lang_name, release, video):
        ...
        logger.debug("bearsub: GET %s params=%r", url, params)
    ...
    def query(self, languages, video):
        ...
        if not imdb_id:
            logger.debug("bearsub: no imdb id on video, skipping")
            return []
        ...
        for lang in languages:
            ...
            if not lang_name:
                logger.debug("bearsub: unsupported language object=%r alpha3=%r", lang, getattr(lang, "alpha3", None))
                continue
            ...
            try:
                ...
                logger.warning(
                    "bearsub: search failed imdb=%s lang=%s release=%s err=%s",
            ...
            for item in results:
                ...
                # Support both new and legacy release fields
                releases = item.get("releases") or []
                if not isinstance(releases, list):
                    releases = [str(releases)] if releases else []
                if not releases and item.get("release_name"):
                    releases = [item["release_name"]]

                # No subscene_link in new API
                page_link = ""

                subtitle = BearSubSubtitle(
                    ...
                    uploader=item.get("source") or item.get("author_name") or "",
                    season=item.get("season"),
                    episode=item.get("episode"),
                    comment=item.get("comment") or item.get("release_name") or "",
                )
